...
property :directory, [String, nil], coerce: proc { |d| d.nil? ? "/var/www/#{name}" : d }
...
property :include_name, [String, nil], default: nil
...
action :create do
  ...
  directory "#{node['nginx']['log_dir']}/#{name}/access" do
    owner 'root'
    group 'root'
    mode 0644
    recursive true
  ...
  directory "#{node['nginx']['log_dir']}/#{name}/error" do
    owner 'root'
    group 'root'
    mode 0644
  end

  include_name = include_name.nil? ? name : include_name
  ...
  declare_resource(:template, "#{node['nginx']['dir']}/sites-available/#{name}.conf") do
    ...
    variables(
      ...
      params: self.class.properties(false)
    )
